...
-- Aggregate pushdown bypasses the NaN/Infinity validation applied to normal
-- row fetches: SUM(bigint) is computed as NUMERIC, which can hold NaN and
-- Infinity, so a pushed-down SUM over such data silently succeeds instead of
-- erroring like a plain SELECT of the same column would.
CREATE FOREIGN TABLE fdw770_special_floats (_id INT, value BIGINT)
  SERVER mongo_server OPTIONS (database 'mongo_fdw_regress', collection 'test_special_floats');
-- Pushed down (default): MongoDB computes 123.456 + NaN, silently returns NaN.
SELECT sum(value) FROM fdw770_special_floats WHERE _id = 1 OR _id = 2;
-- Not pushed down: PostgreSQL fetches and validates each row itself, so the
-- same data now correctly errors.
SET mongo_fdw.enable_aggregate_pushdown TO false;
SELECT sum(value) FROM fdw770_special_floats WHERE _id = 1 OR _id = 2;
SET mongo_fdw.enable_aggregate_pushdown TO true;
-- Same story for Infinity: pushed down silently returns Infinity...
SELECT sum(value) FROM fdw770_special_floats WHERE _id = 1 OR _id = 3;
-- ...but errors when PostgreSQL computes it locally instead.
SET mongo_fdw.enable_aggregate_pushdown TO false;
SELECT sum(value) FROM fdw770_special_floats WHERE _id = 1 OR _id = 3;
SET mongo_fdw.enable_aggregate_pushdown TO true;
...
DROP FOREIGN TABLE fdw757_special_floats;
DROP FOREIGN TABLE fdw770_special_floats;
